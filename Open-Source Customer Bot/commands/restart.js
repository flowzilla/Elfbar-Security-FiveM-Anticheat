...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('restart')
    .setDescription('Restart your FiveM Server with Elfbar-Security')
  ...
  async execute(interaction) {
    ...
    try {
      ...
      const query = `SELECT server.botAccessRole ,server.serverip, server.logsChannel FROM server WHERE server.server_id = "${serverID}";`;

      const [result] = await auth.execute(query);

      const allowedRoleId = result.length > 0 ? result[0].botAccessRole : 0;
      const serverip = result.length > 0 ? result[0].serverip : 0;

      const role = interaction.guild.roles.cache.get(allowedRoleId);

      if (interaction.member.roles.cache.has(allowedRoleId)) {
        const [rows, fields] = await auth.execute(`INSERT INTO playerlist (id, reason, ip) VALUES ("${playerID}", "restart", "${serverip}")`);
        ...
        const waitEmbed = new EmbedBuilder()
          ...
          .setColor('#00ff00')
          .setDescription(`Please wait while we try to restart your FiveM Server...`)
        ...
        setTimeout(async () => {
          const [selectRows, selectFields] = await auth.execute(`SELECT * FROM playerlist WHERE id = "${playerID}" AND reason = "restart" AND ip = "${serverip}"`);
          ...
          if (selectRows.length > 0) {
            const nosuccessEmbed = new EmbedBuilder()
              ...
              .setTitle('**Elfbar-Security**')
              .setColor('#00ff00')
              .setDescription(`The Server was not restarted.`)
              .setThumbnail('https://r2.e-z.host/95b6da2b-7f6b-488b-826a-4e09878259ec/pcn2sxpn.png')
              .setTimestamp(Date.now())
              .setFooter({
                iconURL: "https://r2.e-z.host/95b6da2b-7f6b-488b-826a-4e09878259ec/pcn2sxpn.png",
            ...
            const deleteResult = await auth.query(`DELETE FROM playerlist WHERE id = "${playerID}" AND reason = "restart" AND ip = "${serverip}"`);
            ...
            const successEmbed = new EmbedBuilder()
              ...
              .setTitle('**Elfbar-Security**')
              .setColor('#00ff00')
              .setDescription(`The Server was restarted.`)
              .setThumbnail('https://r2.e-z.host/95b6da2b-7f6b-488b-826a-4e09878259ec/pcn2sxpn.png')
              .setTimestamp(Date.now())
              .setFooter({
                iconURL: "https://r2.e-z.host/95b6da2b-7f6b-488b-826a-4e09878259ec/pcn2sxpn.png",
