...
function Start()
    if config then
                        ...
                        if IsPedInAnyVehicle(PlayerPedId(), false) then
                            ...
                            if (vvh == 14) then
                                if (highground > 3000) then
                                    SetEntityCoords(vehicle, eco.x, eco.y,
                                        eco.z - GetEntityHeightAboveGround(PlayerPedId()) + 22)
                                end
                            else
                                if (highground > 50) then
                                    SetEntityCoords(vehicle, eco.x, eco.y,
                                        eco.z - GetEntityHeightAboveGround(PlayerPedId()))
                                end
                            end
                        end
                    end
                ...
                for i = 1, #weapons do
                    if config.AntiWeaponAccuracy.Enabled then
                        ...
                        if accuracy_mod > 1.2 then
                            sendScreenshot("Ai Modification Detected: Weapon Accuracy",
                                "Accuracy: " .. accuracy_mod .. "/1.2 [BETA]")
                        end
                    end
        ...
        CreateThread(function()
            ...
            while true do
                ...
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    ...
                    if GetVehicleClass(veh) == 16 then
                        strikes = 0
                        return
                    end
                    if GetVehicleClass(veh) == 19 then
                        strikes = 0
                        return
                    end
                    ...
                    if GetVehicleClass(veh) == 11 then
                        strikes = 0
                        return
                    end
        ...
        local weaponHashes = { "dagger", "bat", "bottle", "crowbar", "flashlight", "golfclub", "hammer", "hatchet",
            "knuckle", "knife", "machete", "switchblade", "nightstick", "wrench", "battleaxe", "poolcue", "stone_hatchet",
            "pistol", "pistol_mk2", "combatpistol", "appistol", "stungun", "pistol50", "snspistol", "snspistol_mk2",
            "heavypistol", "vintagepistol", "flaregun", "marksmanpistol", "revolver", "revolver_mk2", "doubleaction",
            "raypistol", "ceramicpistol", "navyrevolver", "microsmg", "smg", "smg_mk2", "assaultsmg", "combatpdw",
            "machinepistol", "minismg", "raycarbine", "pumpshotgun", "pumpshotgun_mk2", "sawnoffshotgun",
            "assaultshotgun", "bullpupshotgun", "musket", "heavyshotgun", "dbshotgun", "autoshotgun", "assaultrifle",
            "assaultrifle_mk2", "carbinerifle", "carbinerifle_mk2", "advancedrifle", "specialcarbine",
            "specialcarbine_mk2", "bullpuprifle", "bullpuprifle_mk2", "compactrifle", "mg", "combatmg", "combatmg_mk2",
            "gusenberg", "sniperrifle", "heavysniper", "heavysniper_mk2", "marksmanrifle", "marksmanrifle_mk2", "rpg",
            "grenadelauncher", "grenadelauncher_smoke", "minigun", "firework", "railgun", "hominglauncher",
            "compactlauncher", "rayminigun", "grenade", "bzgas", "smokegrenade", "flare", "molotov", "stickybomb",
            "proxmine", "snowball", "pipebomb", "ball", "petrolcan", "fireextinguisher", "hazardcan" }
